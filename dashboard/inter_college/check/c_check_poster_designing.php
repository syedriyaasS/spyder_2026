...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['participants']) && isset($_POST['event'])) {
    ...
    $sql = "UPDATE `participants` SET `$column` = NULL WHERE `$column` = 'Poster Designing' AND `id` IN (" . implode(',', $selectedparticipants) . ")";
...
$sqlEvent1 = "SELECT `id`, `name`, `department`, `college`, `email`, `mobile`, `event1`, `event2` FROM `participants` WHERE `event1` = 'Poster Designing'";
$resultEvent1 = $conn->query($sqlEvent1);
$sqlEvent2 = "SELECT `id`, `name`, `department`, `college`, `email`, `mobile`, `event1`, `event2` FROM `participants` WHERE `event2` = 'Poster Designing'";
...
<div class="container">
    <div class="current-page-content">
        ...
        <h2>Poster Designing participants</h2>
