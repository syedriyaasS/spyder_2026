...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['interdepartment']) && isset($_POST['event'])) {
    ...
    $event = $_POST['event'];
    // Prepare the SQL statement to update the specified event fields of interdepartment to NULL for the Group-Dance event
    $column = ($event === 'event1') ? 'event1' : 'event2';
    $sql = "UPDATE `interdepartment` SET `$column` = NULL WHERE `$column` = 'Group Dance' AND `id` IN (" . implode(',', $selectedinterdepartment) . ")";
...
// Filter interdepartment by the 'Group Dance' event in event1 and event2 respectively
$sqlEvent1 = "SELECT `id`, `name`, `department`, `college`, `email`, `mobile`, `event1`, `event2` FROM `interdepartment` WHERE `event1` = 'Group Dance'";
$resultEvent1 = $conn->query($sqlEvent1);
$sqlEvent2 = "SELECT `id`, `name`, `department`, `college`, `email`, `mobile`, `event1`, `event2` FROM `interdepartment` WHERE `event2` = 'Group Dance'";
...
<div class="container">
    <div class="current-page-content">
        ...
        <h2>Group-Dance interdepartment participants</h2>
