...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['interdepartment']) && isset($_POST['event'])) {
    ...
    $sql = "UPDATE `interdepartment` SET `$column` = NULL WHERE `$column` IN ('Biz Masters', 'Marketing') AND `id` IN (" . implode(',', $selectedinterdepartment) . ")";
...
$sqlEvent1 = "SELECT `id`, `name`, `department`, `college`, `email`, `mobile`, `event1`, `event2` FROM `interdepartment` WHERE `event1` IN ('Biz Masters', 'Marketing')";
$resultEvent1 = $conn->query($sqlEvent1);
$sqlEvent2 = "SELECT `id`, `name`, `department`, `college`, `email`, `mobile`, `event1`, `event2` FROM `interdepartment` WHERE `event2` IN ('Biz Masters', 'Marketing')";
...
<div class="container">
    <div class="current-page-content">
        ...
        <h2>Marketing interdepartment participants</h2>
